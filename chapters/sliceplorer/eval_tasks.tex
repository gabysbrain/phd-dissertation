...
\begin{table*}[t]
  \centering
  \caption{
    We summarize our task-based evaluation here. We extended the discrete 
    ...
    We show the scores from our qualitative results inspection as well as
    our expert study on a scale from
    ``none''~\crule{key0}, ``partially''~\crule{key1}, ``mostly''~\crule{key2},
    to ``fully''~\crule{key3}
    where
    ``none'' means that the task is not addressed at all and ``fully'' means 
    that this
    task is directly supported by this view. We provide quotes of
    the general description from Amar, Eagan, and Stasko's paper in the ``discrete''
    column for convenience. 
    \tmnote[nomarginclue]{If there is time left, center the text in the cells properly.}
  }
  \label{tbl:task_list}
...
We first evaluate 1D slices in terms of their flexibility to deal with a broad
...
%~\cite{brehmer:2014}. \tmnote{I would cite Brehmer and Munzner here, or, even
%better, Munzner's book} 
...
%These are the current state of the art visualization techniques that can be
%applied to multi-dimensional continuous data. 
...
% (color-coded scores
%as to how well the techniques address the tasks).  score for each technique as
%to how well it addresses the task. 





%\ttwnote{cite \cite{Isenberg:2013} and the QRI method}
%The evaluation of 1D slices is done in two ways. In this section, we are focusing on a number of low-level tasks and examine how well 1D slices perform. 
%%We evaluate 1D slices with both a low-level task hierarchy and their applicability to high-level tasks.  
%Specifically, we discuss the applicability of 1D slices
%by examining how they address the low-level task taxonomy by Amar and
%Stasko~\cite{Amar:2005}. We extend their taxonomy to specifically address
%continuous data and then demonstrate how flexible 1D slices are for visualizing
%continuous functions.
%
%In \autoref{sec:usage-scenarios}, we are considering a number of high-level task scenarios.
%%On a higher level 
%We then demonstrate how our technique can be applied to answer major
%questions through usage scenarios~\cite{Isenberg:2013}.

%Task taxonomies give us a basis for comparing visualization techniques to each
%other~\cite{Munzner:2014}.
%%~\cite{brehmer:2014}. \tmnote{I would cite Brehmer and Munzner here, or, even better, Munzner's book} 
%If a technique addresses a large number of tasks, that is usually a good
%indicator of its flexibility. However, to the best of our knowledge, there is
%no low-level taxonomy focused on the visual analysis of multi-dimensional
%continuous data. So, we took a popular taxonomy for tasks on discrete data, the one by Amar and
%Stasko~\cite{Amar:2005} and extended each of their task categories to directly
%address continuous data. We then use it to compare 1D slices to other state of the
%art techniques for multi-dimensional continuous data: HyperSlice~\cite{Wijk:1993}, topological
%spines~\cite{Correa:2011}, contour trees~\cite{Carr:2003}, and the technique by
%Gerber et al.~\cite{Gerber:2010} (see \autoref{fig:compare}). 
%We refer to topological spines, contour trees, and the work by Gerber 
%et al.\ as topological techniques when it makes sense to compare them as
%a group.
%%These are the current state of the art visualization techniques that can be
%%applied to multi-dimensional continuous data. 
%We evaluate based on all tasks, except for ``sort'',
%for which we could not find a suitable extension to continuous functions.
%The extensions are shown in
%\autoref{tbl:task_list} along with the score for each technique as to how well
%it addresses the task. 
%The guiding theme in our extensions is that users want
%to view the relationship of independent variables to the dependent variable and
%to see how the dependent variable changes with respect to the independent values. 
%
%% From related work merged in here
...
%\msnote{if we do the 5 people judgment, we should take this out again, then it is not a QRI anymore}
%Since there are existing state-of-the art
%methods employed to address these tasks we adopt a comparative strategy. 
...
%\ttwnote{link?} \msnote{for now,
%include in supplemental material, and add a footnote saying that the webpage
%will be put on our university server upon final acceptance}
