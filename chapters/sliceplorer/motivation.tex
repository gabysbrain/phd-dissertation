...
%want to examine the results of a prediction model and find a what parameter
%settings produce robust optima \msnote{add aspect of continuous}. 
...
%There are a number of techniques that we will discuss
%in more detail in the background section (\autoref{sec:background}). 
...
local detail.
%\msnote{this is a nice paragraph!}

% TM: this figure doesn't work for me right now (and it isn't integrated into the paragraph), hence I am taking it out.
%\begin{figure}
%  \centering
%  \includegraphics[width=8cm]{images/technique_scale}
%  \caption{
%  }
%  \label{fig:scale}
%\end{figure}
...
and projection techniques. %Classical projections (scatterplots), which would equal ``0D slices'' in the above terminology, have been hugely successful in the discrete data realm. What we are proposing here are 1D projections for the continuous data realm.

%Naturally, our technique fits into the currently developed techniques for
...
%Although 2D and 3D scalar functions are also multi-dimensional functions, our
%technique is focused on functions with domains of \emph{4D and higher}. 
...
three usage scenarios illustrating the value of 1D slices in common function 
      visualization scenarios. % in both computational and data science applications.
      %, and
%\item a discussion of task analysis for multi-dimensional functions. 
%\item and deriving design guidelines based on the results of our evaluation. \msnote{do we still do that?}
\end{itemize}
