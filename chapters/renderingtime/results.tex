...
\begin{figure*}[t]
  ...
  \caption{
    ...
    formula. 
    %The colours represent different values for the number of sample points. 
    %The hockey stick shape is due to whether the filtering time 
    %or drawing time is dominant.
  }
...
To fit these data we first computed the fragments and rendering time per 
...
%Our
%``fragments drawn'' measure in \autoref{fig:hs_draw_times} is based on calculating
%the size of the geometry to be rendered \emph{before} rasterization by the GPU.
